...
df <- as.data.frame(df)
names(df) <- c("k", "OC1", "OC3", "OC2", "EL")
df <- melt(df, "k")
df$se <- sqrt(df$value*(1-df$value)/10000)

ggplot(df, aes(k, value, colour=variable)) + geom_point() + geom_line() +
  geom_errorbar(aes(ymin=value-qnorm(0.975)*se, ymax=value+qnorm(0.975)*se), width=.2) +
  theme_minimal()
```